...
private class CumulativeHistogramGenerator(parent: ExperimentGenerator, item: CumulativeHistogramItem, id: Int)
  ...
  protected class RunBinding(val vars: Buffer[Dynamics[Double]],
                             ...
                             val data: Buffer[(Int, Buffer[Double])],
                             var file: File)
  ...
  private def processSimulationFinished(run: RunBinding): Disposable = {
    ...
    experiment.simulation.finished subscribe ((unit) => {
      ...
      def series(i: Int): Array[Double] = {

        val xs = for ((_, x) <- b) yield x(i)
      ...
      try {
        ...
        for (i <- 0 to run.names.length - 1) {
          s.addSeries(run.names(i), series(i).toArray, item.value.bins)
        }
