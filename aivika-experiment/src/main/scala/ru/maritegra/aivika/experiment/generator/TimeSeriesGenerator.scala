...
private class TimeSeriesGenerator(parent: ExperimentGenerator, item: TimeSeriesItem, id: Int)
      ...
      try {
        ...
        if (run.names.length > 0) {
          ...
          run.vars(0) match {
            ...
            case x: Stateful => p.setRenderer(0, new XYStepRenderer)
            case _ => p.setRenderer(0, new StandardXYItemRenderer)
          }
        }
        ...
        for (i <- 1 to run.names.length - 1) {
          ...
          run.vars(i) match {
            ...
            case x: Stateful => p.setRenderer(i, new XYStepRenderer)
            case _ => p.setRenderer(i, new StandardXYItemRenderer)
          }
        }
  ...
  private def add(run: RunBinding, t: Double, i: Int, x: Double) {
    run.data(i).add(t, x)
  }
