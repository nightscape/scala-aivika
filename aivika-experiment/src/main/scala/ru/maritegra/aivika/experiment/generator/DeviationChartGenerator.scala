...
private class DeviationChartGenerator(parent: ExperimentGenerator, item: DeviationChartItem, id: Int)
  ...
  protected class RunBinding(val vars: Buffer[Dynamics[Double]],
                             ...
                             val data: Buffer[Buffer[SamplingStatistics]],
                             var file: File)
  ...
  private def processSimulationStarted(run: RunBinding): Disposable = {
    ...
    for (i <- 0 to vs.length - 1) {
      ...
      val ts = Buffer[SamplingStatistics]()

      for (n <- 0 to experiment.specs.iterations - 1) {
  ...
  private def processSimulationFinished(run: RunBinding): Disposable = {
    ...
    experiment.simulation.finished subscribe ((unit) => {
      ...
      def series(i: Int): YIntervalSeries = {

        val r = new YIntervalSeries(run.names(i))

        for (n <- 0 to experiment.specs.iterations - 1) {

          val ts = run.data(i)(n)

          val x = experiment.specs.time(n, 0)
          val y = ts.mean
          val sigma = math.sqrt(ts.variance)

          r.add(x, y, y - 3 * sigma, y + 3 * sigma)
        }
      ...
      try {
        ...
        for (i <- 1 to run.names.length - 1) {
          ...
          item.value.yAxis match {
            ...
            case SingleAxis(_) =>
              p.setDataset(i, new YIntervalSeriesCollection { addSeries(series(i)) })
          ...
          p.setRenderer(i, new DeviationRenderer(true, false))
        }
