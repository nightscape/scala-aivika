/*
 * Copyright (C) 2009-2012 David Sorokin <david.sorokin@gmail.com>.
 *
 * All rights reserved.
 *
 * Licensed under BSD3. See LICENSE for details.
 */

package ru.maritegra.aivika.experiment

import ru.maritegra.aivika._
import ru.maritegra.aivika.SystemDynamics._

class CumulativeXYChart {

  var title = "Cumulative XY Chart"

  var description = ""

  var file: ItemFile = UniqueFile("$TITLE", ".png")

  var width = 800

  var height = 500

  var legend = true

  var tooltips = true

  var renderer: XYRenderer = StandardXYRenderer

  var yAxis: ItemAxis = MultipleAxis

  var filter: Dynamics[Boolean] = Dynamics(true)

  var x: DynamicsSeries[Double] = Series("t", time)

  val ys = new SeriesBuffer[Double]
}