...
private class PieChartGenerator(parent: ExperimentGenerator, item: PieChartItem, id: Int)
  ...
  protected def initRun(runIndex: Int): RunBinding = {
    new RunBinding(runIndex, Buffer(), Buffer(), null)
  }
  ...
  private def processLastPoint(run: RunBinding): Disposable = {
    ...
    experiment.simulation.afterLastPointInRun(run.index) subscribe ((p: Point) => {
      ...
      val c = ChartFactory.createPieChart(item.value.title, data,
