...
private trait CumulativeBindableSeries[A] {
  ...
  private var binding: Option[(RunRep, Map[Series[A], Binding])] = None
  ...
  def toHtml: Seq[Node] = binding match {

    case None => List()
    case Some((run, bs)) => toHtml(run)
  }

  protected def toHtml(run: RunRep): Seq[Node]
}
